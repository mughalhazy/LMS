"""SMS communication adapter stub.

B05-005: communication-adapter-contract.md — required channel "sms".
...
logger = logging.getLogger("integrations.communication.sms")


class SmsAdapter(CommunicationAdapter):
    """SMS channel adapter — satisfies CommunicationAdapter contract."""
    ...
    def send_message(self, *, channel: str, recipient_id: str, recipient_address: str,
        ...
        logger.info("SMS send to %s (tenant=%s) idempotency=%s", recipient_address, tenant_id, idempotency_key)
        return self._make_result("sms", state="queued")
    ...
    def schedule_message(self, *, channel: str, recipient_id: str, recipient_address: str,
        ...
        logger.info("SMS scheduled to %s at %s (tenant=%s)", recipient_address, send_at, tenant_id)
        return self._make_result("sms", state="scheduled")

    def broadcast(self, *, channel: str, recipients: list[dict[str, Any]], body: str,
                  tenant_id: str, **kwargs: Any) -> AdapterResult:
        logger.info("SMS broadcast to %d recipients (tenant=%s)", len(recipients), tenant_id)
        return self._make_result("sms", state="accepted")
