...
export interface LessonContentRepository {
  getContentByLessonId(lessonId: UUID): Promise<LessonContentItem[]>;
  ...
  findContentById(lessonContentId: UUID): Promise<LessonContentItem | null>;

  getResourcesByLessonId(lessonId: UUID): Promise<LessonResource[]>;
  ...
  findResourceById(resourceId: UUID): Promise<LessonResource | null>;
}
...
export class InMemoryLessonContentRepository implements LessonContentRepository {
  ...
  async getContentByLessonId(lessonId: UUID): Promise<LessonContentItem[]> {
    return this.content
  ...
  async updateContent(item: LessonContentItem): Promise<LessonContentItem> {
    const index = this.content.findIndex((entry) => entry.lessonContentId === item.lessonContentId);
    if (index >= 0) {
  ...
  async findContentById(lessonContentId: UUID): Promise<LessonContentItem | null> {
    return this.content.find((item) => item.lessonContentId === lessonContentId) ?? null;
  }

  async getResourcesByLessonId(lessonId: UUID): Promise<LessonResource[]> {
  ...
  async updateResource(resource: LessonResource): Promise<LessonResource> {
    const index = this.resources.findIndex((entry) => entry.resourceId === resource.resourceId);
    if (index >= 0) {
  ...
  async findResourceById(resourceId: UUID): Promise<LessonResource | null> {
    return this.resources.find((resource) => resource.resourceId === resourceId) ?? null;
  }
}
