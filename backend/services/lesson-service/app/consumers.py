"""Event consumers for lesson-service (FA-024, G-24).
...
logger = logging.getLogger("lesson-service.consumers")

def _on_course_published(event: EventEnvelope) -> None:
    """On lms.course.published.v1 — log; future: mark lessons deliverable."""
    logger.info("lesson-service: lms.course.published.v1 received — tenant=%s", event.tenant_id)

def _on_course_archived(event: EventEnvelope) -> None:
    """On lms.course.archived.v1 — log; future: freeze lesson edits."""
    logger.info("lesson-service: lms.course.archived.v1 received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all lesson-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("lms.course.published.v1", _on_course_published)
    bus.subscribe("lms.course.archived.v1", _on_course_archived)
    logger.info("lesson-service: event consumers registered")
