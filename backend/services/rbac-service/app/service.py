...
from .models import AuthorizationDecisionLog, PolicyRule, RoleDefinition, RoleStatus, ScopeType, SubjectRoleAssignment
from .schemas import (
...
class RBACService:
    ...
    # ------------------------------------------------------------------
    # BC-BRANCH-01: Multi-branch enforcement — MO-026 / Phase C
    # hq_admin / hq_viewer: TENANT-scoped assignment → cross-branch access.
    # branch_manager / branch_staff: BRANCH-scoped assignment → restricted to
    #   their branch_ids list. require_branch_access() raises 403 if the
    #   subject lacks access to the requested branch.
    # ------------------------------------------------------------------

    def get_effective_branch_ids(
        self,
        tenant_id: str,
        subject_type: str,
        subject_id: str,
    ) -> list[str] | None:
        """Return branch IDs the subject can access, or None for tenant-wide access.

        Returns:
            None  — subject has TENANT-scoped assignment; can see all branches.
            list  — subject has BRANCH-scoped assignments; restricted to these IDs.
            []    — no active assignments at all; no branch access.
        """
        now = datetime.now(timezone.utc)
        active = [
            a for a in self.storage.list_assignments(tenant_id)
            if a.subject_type.value == subject_type
        ...
        # If any active assignment is TENANT-scoped → tenant-wide / HQ access
        for a in active:
            if a.scope_type == ScopeType.TENANT:
                return None  # None = unrestricted

        # Collect branch_ids from all BRANCH-scoped assignments
        branch_ids: set[str] = set()
        for a in active:
            if a.scope_type == ScopeType.BRANCH and a.branch_ids:
                branch_ids.update(a.branch_ids)
    ...
    def require_branch_access(
        ...
        """Raise 403 if subject lacks access to the given branch (BC-BRANCH-01).

        HQ users (TENANT-scoped) always pass. Branch users must have the
        branch_id in their effective branch list.
        """
        effective = self.get_effective_branch_ids(tenant_id, subject_type, subject_id)
        if effective is None:
            return  # HQ / tenant-wide — always allowed
        if branch_id not in effective:
            raise HTTPException(
                status_code=403,
                detail={
                    "error": "branch_access_denied",
                    "branch_id": branch_id,
                    "reason": "BC-BRANCH-01: subject not assigned to this branch",
                },
            )
