...
class ScopeType(str, Enum):
    ...
    PROGRAM = "program"
    COHORT = "cohort"
    # BC-BRANCH-01 / MO-026: branch scope for multi-branch operators.
    # Assignments with scope_type=BRANCH use branch_ids list (not scope_id).
    # hq_admin/hq_viewer use scope_type=TENANT with cross-branch visibility.
    BRANCH = "branch"
...
class SubjectRoleAssignment(BaseModel):
    ...
    scope_id: str
    # BC-BRANCH-01 / MO-026: for scope_type=BRANCH assignments, branch_ids lists
    # the branches this subject is permitted to operate within.
    # Empty list = no branch access. TENANT-scoped assignments leave this None.
