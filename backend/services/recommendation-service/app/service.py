...
class RecommendationService:
    def __init__(self) -> None:
        ...
        # CGAP-072: per-learner analytics signals consumed by personalization scoring.
        # Keys: (tenant_id, learner_id) → signal dict with completion_rate,
        # engagement_score, trend_direction, skill_history.
        self._analytics_signals: dict[tuple[str, str], dict] = {}

    def ingest_analytics_signal(
        self,
        *,
        tenant_id: str,
        learner_id: str,
        ...
        """CGAP-072: store an analytics signal for use in personalized scoring.

        Expected signal keys (all optional):
          completion_rate (0–100), engagement_score (0–100),
          trend_direction ("up"|"down"|"stable"), skill_history (list of skill_ids).
        """
        self._analytics_signals[(tenant_id.strip(), learner_id.strip())] = dict(signal)
    ...
    def generate_personalized_courses(
        ...
        # CGAP-072: use real analytics signals when available; fall back to
        # available_minutes heuristic only when no history is present.
        signal = self._analytics_signals.get((req.tenant_id.strip(), req.learner_id.strip()))
        if signal:
            engagement_factor = signal.get("engagement_score", 50) / 100
            completion_factor = signal.get("completion_rate", 50) / 100
            trend_boost = (
                0.07 if signal.get("trend_direction") == "up"
                else -0.05 if signal.get("trend_direction") == "down"
                else 0.0
            )
            base_score = min(1.0, max(0.1,
                0.35
                + (engagement_factor * 0.30)
                + (completion_factor * 0.20)
                + (req.available_minutes_per_week / 600 * 0.15)
                + trend_boost
            ))
        else:
