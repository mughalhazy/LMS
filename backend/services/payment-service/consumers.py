"""Event consumers for payment-service (FA-024, G-24).
...
logger = logging.getLogger("payment-service.consumers")


def _on_enrollment_changed(event: EventEnvelope) -> None:
    """On enrollment.lifecycle.changed — log; future: trigger payment reconciliation."""
    logger.info("payment-service: enrollment.lifecycle.changed received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all payment-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("enrollment.lifecycle.changed", _on_enrollment_changed)
    logger.info("payment-service: event consumers registered")
