"""Event consumers for invoice-billing-service (FA-024, G-24).
...
logger = logging.getLogger("invoice-billing-service.consumers")

def _on_enrollment_changed(event: EventEnvelope) -> None:
    """On enrollment.lifecycle.changed — log; future: generate invoice line item."""
    logger.info("invoice-billing-service: enrollment.lifecycle.changed received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all invoice-billing-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("enrollment.lifecycle.changed", _on_enrollment_changed)
    logger.info("invoice-billing-service: event consumers registered")
