"""Event consumers for usage-metering-service (FA-024, G-24).
...
logger = logging.getLogger("usage-metering-service.consumers")

def _on_enrollment_changed(event: EventEnvelope) -> None:
    """On enrollment.lifecycle.changed — log; future: record usage event."""
    logger.info("usage-metering-service: enrollment.lifecycle.changed received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all usage-metering-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("enrollment.lifecycle.changed", _on_enrollment_changed)
    logger.info("usage-metering-service: event consumers registered")
