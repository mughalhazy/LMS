...
class HTTPServiceDataProvider:
    ...
    def _get_json(self, url: str, tenant_id: str) -> dict[str, Any]:
        """CGAP-061: fetch JSON from upstream service with degraded-mode fallback.

        On any HTTP error, returns a sentinel dict with `_degraded=True` and
        `_degraded_reason` so callers can detect unavailability and surface a
        meaningful fallback response instead of silently using empty context.
        """
        ...
        try:
            ...
            return {"_degraded": True, "_degraded_reason": "upstream_timeout"}
        except httpx.HTTPStatusError as exc:
            return {"_degraded": True, "_degraded_reason": f"upstream_http_{exc.response.status_code}"}
        except httpx.HTTPError:
            return {"_degraded": True, "_degraded_reason": "upstream_unavailable"}
...
class AITutorService:
    ...
    @staticmethod
    def _is_degraded(data: dict[str, Any]) -> bool:
        """CGAP-061: detect degraded-mode sentinel from _get_json()."""
        return bool(data.get("_degraded"))

    @staticmethod
    def _degraded_fallback(service_name: str) -> dict[str, Any]:
        """CGAP-061: return a named degraded sentinel so tutor responses can surface the outage."""
        return {"_degraded": True, "_degraded_service": service_name, "_degraded_reason": "service_unavailable"}

    def _build_context_bundle(self, tenant_id: str, learner_id: str, course_id: str) -> TutorContextBundle:
        """CGAP-061: build context bundle with degraded-mode awareness.

        When upstream services are unavailable, returns a bundle with sentinel data
        so the tutor can surface a meaningful degraded response rather than silently
        using empty context. Callers should check `bundle.is_degraded` before use.
        """
        course_data = self._data_provider.get_course(tenant_id=tenant_id, course_id=course_id)
        if self._is_degraded(course_data):
            course_data = self._degraded_fallback("course-service")
        ...
        if self._is_degraded(progress_data):
            progress_data = self._degraded_fallback("progress-service")
        ...
        if self._is_degraded(analytics_data):
            analytics_data = self._degraded_fallback("analytics-service")
    ...
    # MS-AI-01 (MS§10.8): map interaction type to guidance level.
    # Explanations are informational; tutoring/guidance are advisory;
    # question answers are suggestions (learner-contextual, not authoritative).
    _GUIDANCE_LEVEL_MAP: dict[InteractionType, str] = {
        InteractionType.EXPLANATION: "informational",
        InteractionType.QUESTION: "suggestion",
        InteractionType.CONTEXTUAL_TUTORING: "advisory",
        InteractionType.GUIDANCE: "advisory",
    }
    ...
    def _store_interaction(
        ...
        interaction = TutorResponse(
            ...
            # MS-AI-01 (MS§10.8): label output, declare guidance level, expose override path
            ai_generated=True,
            guidance_level=self._GUIDANCE_LEVEL_MAP.get(interaction_type, "suggestion"),
            human_review_available=True,
            override_path="support://request-human-review",
        )
