"""Event consumers for checkout-service (FA-024, G-24).
...
logger = logging.getLogger("checkout-service.consumers")

def _on_enrollment_changed(event: EventEnvelope) -> None:
    """On enrollment.lifecycle.changed — log; future: reconcile checkout state."""
    logger.info("checkout-service: enrollment.lifecycle.changed received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all checkout-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("enrollment.lifecycle.changed", _on_enrollment_changed)
    logger.info("checkout-service: event consumers registered")
