...
class CourseService:
    ...
    def update_course(self, course_id: str, request: UpdateCourseRequest) -> CourseResponse:
        ...
        # CGAP-065: snapshot current state into version_history before mutating.
        snapshot = {
            "version": record.version,
            "snapshotted_at": self._now().isoformat(),
            "snapshotted_by": request.updated_by,
            "title": record.title,
        ...
        record.version_history.append(snapshot)
        record.version += 1
    ...
    def get_version_history(self, tenant_id: str, course_id: str) -> list[dict]:
        """CGAP-065: return the version history snapshots for a course."""
        record = self._get_tenant_course(tenant_id, course_id)
        return list(record.version_history)

    def rollback_to_version(self, tenant_id: str, course_id: str, target_version: int, rolled_back_by: str) -> CourseResponse:
        """CGAP-065: restore a course to a prior version snapshot.

        Appends a new version_history entry for the rollback action.
        The restored state becomes the next incremented version.
        """
        record = self._get_tenant_course(tenant_id, course_id)
        snapshot = next((s for s in record.version_history if s["version"] == target_version), None)
        if snapshot is None:
        ...
        pre_rollback = {
            ...
            "credit_value": record.credit_value,
            "grading_scheme": record.grading_scheme,
            "updated_fields": ["rollback"],
        }
        record.version_history.append(pre_rollback)

        # Restore fields from snapshot
        for field_name in ["title", "description", "course_code", "language_code", "credit_value", "grading_scheme"]:
            if field_name in snapshot:
                setattr(record, field_name, snapshot[field_name])
        record.version += 1
        record.updated_at = self._now()
        record.version_history.append({
            "version": record.version,
            "snapshotted_at": record.updated_at.isoformat(),
            "snapshotted_by": rolled_back_by,
            "title": record.title,
            "updated_fields": [f"rollback_to_v{target_version}"],
        })
        self.storage.save(record)
        self.audit_logger.log(
            event_type="course.version.rolled_back",
            tenant_id=tenant_id,
