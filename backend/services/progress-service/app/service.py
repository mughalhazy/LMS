...
class ProgressService:
    def __init__(
        ...
        # BC-RISK-01 (CGAP-062): thresholds for proactive risk detection on course progress.
        # Keys: risk_level → completion_percentage ceiling (exclusive upper bound).
        self._risk_thresholds: dict[str, float] = {
            "critical": 40.0,   # completion < 40 % → critical risk
            "elevated": 60.0,   # completion 40–60 % → elevated risk
        }
    ...
    def upsert_lesson_progress(self, lesson_id: str, request: LessonProgressUpsertRequest, actor_id: str) -> ProgressRecordResponse:
        ...
        self._check_and_push_risk_insight(request.tenant_id, request.learner_id, request.course_id, snapshot)
    ...
    def _check_and_push_risk_insight(
        self,
        tenant_id: str,
        learner_id: str,
        course_id: str,
        ...
        """BC-RISK-01 (CGAP-062): emit progress.risk.detected when completion falls below threshold.

        Only fires when at least one lesson has been attempted (total_lessons > 0) and the
        course is not yet completed — avoids false positives on brand-new enrolments or
        already-completed courses. Best-effort delivery via shared event bus.
        """
        if snapshot.completion_status == "completed" or snapshot.total_lessons == 0:
            return

        pct = snapshot.progress_percentage
        risk_level: str | None = None
        if pct < self._risk_thresholds["critical"]:
            risk_level = "critical"
        elif pct < self._risk_thresholds["elevated"]:
            risk_level = "elevated"
        ...
        event_payload = {
            "event_type": "progress.risk.detected",
            "tenant_id": tenant_id,
            "learner_id": learner_id,
            "course_id": course_id,
            "enrollment_id": snapshot.enrollment_id,
            "completion_percentage": pct,
            "risk_dimension": "completion",
        ...
        try:
            ...
            pass  # best-effort — event bus unavailable must not block progress writes
