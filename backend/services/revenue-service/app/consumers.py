"""Event consumers for revenue-service (FA-024, G-24).
...
logger = logging.getLogger("revenue-service.consumers")

def _on_enrollment_changed(event: EventEnvelope) -> None:
    """On enrollment.lifecycle.changed — log; future: record revenue signal."""
    logger.info("revenue-service: enrollment.lifecycle.changed received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all revenue-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("enrollment.lifecycle.changed", _on_enrollment_changed)
    logger.info("revenue-service: event consumers registered")
