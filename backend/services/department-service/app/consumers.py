"""Event consumers for department-service (FA-024, G-24).
...
logger = logging.getLogger("department-service.consumers")

def _on_user_status_changed(event: EventEnvelope) -> None:
    """On lms.user.status.changed — log; future: update department member status."""
    logger.info("department-service: lms.user.status.changed received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all department-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("user.status_changed", _on_user_status_changed)
    logger.info("department-service: event consumers registered")
