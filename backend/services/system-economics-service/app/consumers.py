"""Event consumers for system-economics-service (FA-024, G-24).
...
logger = logging.getLogger("system-economics-service.consumers")

def _on_enrollment_changed(event: EventEnvelope) -> None:
    """On enrollment.lifecycle.changed — log; future: compute system cost signal."""
    logger.info("system-economics-service: enrollment.lifecycle.changed received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all system-economics-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("enrollment.lifecycle.changed", _on_enrollment_changed)
    logger.info("system-economics-service: event consumers registered")
