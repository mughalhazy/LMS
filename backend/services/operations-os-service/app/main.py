...
from .service import OperationsOSService
...
class OpsOSHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        p = self._base()
        if p not in _OPS_EXEMPT and not _jwt_valid(self.headers.get("Authorization")):
            self._send(401, {"error": "unauthorized"}); return
        tid = self.headers.get("X-Tenant-Id", "")
        oid = self.headers.get("X-Operator-Id", "")
        if p == "/health":
            self._send(200, {"status": "ok", "service": "operations-os-service"}); return
        if p == "/api/v1/ops/daily-action-list":
            self._send(*SERVICE.get_daily_action_list(oid, tid, self._qs("date"))); return
        if p == "/api/v1/ops/action-queue":
            self._send(*SERVICE.get_action_queue(oid, tid)); return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if p == "/api/v1/ops/metrics":
                self._send(*SERVICE.record_metric(body)); return
            self._send(404, {"error": "not_found"})
        except (TypeError, KeyError, json.JSONDecodeError) as exc:
            self._send(400, {"error": str(exc)})


def run(host: str = "0.0.0.0", port: int = 8108) -> None:
    server = HTTPServer((host, port), OpsOSHandler)
    print(f"Operations OS service listening on http://{host}:{port}")
