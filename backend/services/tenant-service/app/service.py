...
def _resolve_plan_capabilities_from_registry(plan_type: str) -> set[str] | None:
    """CGAP-069: query the authoritative capability registry for plan capabilities.

    Replaces the env-var bypass with the canonical entitlement chain. Returns None
    when the registry is unavailable so callers can fall back to the env-var mapping.
    """
    try:
        import sys
        from pathlib import Path
        ...
        from services.subscription_service import SubscriptionService as _SvcClass  # type: ignore[import]
        svc = _SvcClass()
        caps = svc.get_plan_capabilities(plan_type.strip().lower())
        if isinstance(caps, (set, frozenset, list, tuple)):
            return set(caps)
        return None
    except Exception:
        # Registry unavailable — fallback to env-var mapping in caller
        return None
...
class TenantService:
    def __init__(self, store: TenantStore, publisher: EventPublisher, metrics: MetricsRegistry):
        self.store = store
        # CGAP-069: cached plan→capabilities mapping; registry is queried first, env-var used as fallback.
        self._plan_capabilities_cache: dict[str, set[str]] = {}
    ...
    def get_plan_capabilities(self, plan_type: str) -> set[str]:
        """CGAP-069: resolve plan capabilities via capability registry (authoritative), falling back
        to SUBSCRIPTION_PLAN_CAPABILITIES_JSON env var when registry is unavailable.

        This replaces the direct env-var read that bypassed the entitlement chain.
        """
        normalized = plan_type.strip().lower()
        if normalized in self._plan_capabilities_cache:
            return self._plan_capabilities_cache[normalized]

        # 1. Try authoritative registry
        caps = _resolve_plan_capabilities_from_registry(normalized)
        if caps is not None:
            self._plan_capabilities_cache[normalized] = caps
            return caps

        # 2. Fall back to env-var mapping (backward compat)
        env_mapping = _plan_capability_mapping()
        fallback = env_mapping.get(normalized, set())
        self._plan_capabilities_cache[normalized] = fallback
        return fallback
