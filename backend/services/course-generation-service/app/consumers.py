"""Event consumers for course-generation-service (FA-024, G-24).
...
logger = logging.getLogger("course-generation-service.consumers")

def _on_course_published(event: EventEnvelope) -> None:
    """On lms.course.published.v1 — log; future: close generation task."""
    logger.info("course-generation-service: lms.course.published.v1 received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all course-generation-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("lms.course.published.v1", _on_course_published)
    logger.info("course-generation-service: event consumers registered")
