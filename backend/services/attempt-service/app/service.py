...
# CGAP-067: import publish_event from shared transport layer.
# Adds backend/ to sys.path so backend.services.shared.events is importable.
_BACKEND_ROOT = Path(__file__).resolve().parents[3]
if str(_BACKEND_ROOT) not in sys.path:
    sys.path.insert(0, str(_BACKEND_ROOT))

try:
    from services.shared.events.envelope import publish_event as _publish_event_to_bus
except ImportError:
    _publish_event_to_bus = None  # type: ignore[assignment]
from .schemas import (
    AnswerSubmission,
...
class AttemptService:
    ...
    def score_attempt(self, attempt_id: str, request: ScoreAttemptRequest) -> AttemptResponse:
        ...
        # CGAP-067: BC-WF-01 — emit assessment.failed to the platform event bus
        # so the workflow engine's wf_default_assessment_failed workflow can fire.
        if not attempt.passed and _publish_event_to_bus is not None:
            try:
                _publish_event_to_bus(
                    event_type="assessment.failed",
                    producer_service="attempt-service",
                ...
                pass  # Event emission must not fail the scoring operation
