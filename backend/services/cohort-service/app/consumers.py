"""Event consumers for cohort-service (FA-024, G-24).
...
logger = logging.getLogger("cohort-service.consumers")

def _on_enrollment_changed(event: EventEnvelope) -> None:
    """On enrollment.lifecycle.changed — log; future: sync cohort membership count."""
    logger.info("cohort-service: enrollment.lifecycle.changed received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all cohort-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("enrollment.lifecycle.changed", _on_enrollment_changed)
    logger.info("cohort-service: event consumers registered")
