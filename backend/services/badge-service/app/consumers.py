"""Event consumers for badge-service (FA-024, G-24).
...
logger = logging.getLogger("badge-service.consumers")

def _on_assessment_graded(event: EventEnvelope) -> None:
    """On assessment.graded.v1 — log; future: evaluate badge criteria."""
    logger.info("badge-service: assessment.graded.v1 received — tenant=%s", event.tenant_id)

def _on_enrollment_changed(event: EventEnvelope) -> None:
    """On enrollment.lifecycle.changed — log; future: evaluate completion badge."""
    logger.info("badge-service: enrollment.lifecycle.changed received — tenant=%s", event.tenant_id)


def register_consumers() -> None:
    """Register all badge-service consumers on the default event bus."""
    bus = get_default_bus()
    bus.subscribe("assessment.graded", _on_assessment_graded)
    bus.subscribe("enrollment.lifecycle.changed", _on_enrollment_changed)
    logger.info("badge-service: event consumers registered")
