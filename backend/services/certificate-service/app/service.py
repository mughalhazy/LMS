...
class CertificateApplicationService:
    ...
    def handle_enrollment_completed(self, event: dict[str, Any]) -> Certificate | None:
        """CGAP-077: Handle enrollment.completed events by triggering certificate issuance.

        Extracts tenant_id, user_id, course_id, and enrollment_id from the event payload.
        Uses a minimal completion_ref derived from the event.  If a certificate already exists
        for this enrollment (idempotent re-delivery), the method returns None silently.
        """
        payload = event.get("payload", event)  # support both raw payload and envelope wrappers
        tenant_id = str(payload.get("tenant_id", "")).strip()
        user_id = str(payload.get("user_id", "")).strip()
        course_id = str(payload.get("course_id", "")).strip()
        enrollment_id = str(payload.get("enrollment_id", "")).strip() or None
        completed_at = payload.get("completed_at") or datetime.utcnow().isoformat()

        if not (tenant_id and user_id and course_id):
            return None  # malformed event — nothing to issue

        # Resolve template: prefer event-supplied template_id, fall back to tenant default sentinel.
        template_id = str(payload.get("template_id", "")).strip() or f"default__{tenant_id}"
        ...
        request: dict[str, Any] = {
            ...
            "issued_by": "system:enrollment.completed",
            "metadata": payload.get("metadata", {}),
            "completion_ref": {
                "source_event": "enrollment.completed",
                "enrollment_id": enrollment_id or "",
                "completed_at": completed_at,
            },
        }
        try:
            return self.issue_certificate(tenant_id=tenant_id, request=request)
        except DomainError as exc:
            if exc.code == "CERTIFICATE_ALREADY_EXISTS":
                return None  # idempotent — already issued for this enrollment
            raise
