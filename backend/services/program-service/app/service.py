...
class ProgramService:
    ...
    def create_program(self, request: CreateProgramRequest, correlation_id: str | None = None) -> ProgramResponse:
        ...
        self.audit_logger.log(event_type="program.creation", tenant_id=request.tenant_id, correlation_id=correlation_id, actor_id=request.created_by, details={"program_id": program.program_id, "code": request.code})
        self.event_publisher.publish(event_type="lms.program.program_created.v1", tenant_id=request.tenant_id, correlation_id=correlation_id, payload={"program_id": program.program_id, "institution_id": request.institution_id, "code": request.code, "status": program.status.value, "version": 1, "created_at": now.isoformat()})
        self.observability.increment("program_create_total")
        return self._program_response(program)

    def get_program(self, tenant_id: str, program_id: str, correlation_id: str | None = None) -> ProgramDetailResponse:
    ...
    def list_programs(self, tenant_id: str, institution_id: str | None, status: ProgramStatus | None, page: int, page_size: int, correlation_id: str | None = None) -> ProgramListResponse:
        ensure_correlation_id(correlation_id)
    ...
    def update_program(self, program_id: str, request: UpdateProgramRequest, correlation_id: str | None = None) -> ProgramUpdateResult:
        correlation_id = ensure_correlation_id(correlation_id)
        ...
        self.audit_logger.log(event_type="program.updated", tenant_id=program.tenant_id, correlation_id=correlation_id, actor_id=request.updated_by, details={"program_id": program_id, "updated_fields": updated_fields, "version": program.version})
        self.event_publisher.publish(event_type="lms.program.program_updated.v1", tenant_id=program.tenant_id, correlation_id=correlation_id, payload={"program_id": program.program_id, "updated_fields": updated_fields, "version": program.version, "updated_at": program.updated_at.isoformat()})
    ...
    def transition_status(self, program_id: str, request: TransitionProgramStatusRequest, correlation_id: str | None = None) -> StatusTransitionResponse:
        correlation_id = ensure_correlation_id(correlation_id)
        program = self._get_tenant_program(request.tenant_id, program_id)
        ...
        self.store.update(program)
        self.audit_logger.log(event_type="program.status.changed", tenant_id=program.tenant_id, correlation_id=correlation_id, actor_id=request.changed_by, details={"program_id": program.program_id, "from_status": from_status.value, "to_status": request.target_status.value, "reason": request.change_reason})
        self.event_publisher.publish(event_type="lms.program.program_status_changed.v1", tenant_id=program.tenant_id, correlation_id=correlation_id, payload={"program_id": program.program_id, "from_status": from_status.value, "to_status": request.target_status.value, "change_reason": request.change_reason, "changed_by": request.changed_by, "changed_at": changed_at.isoformat()})
    ...
    def upsert_institution_link(self, program_id: str, request: UpsertInstitutionLinkRequest, correlation_id: str | None = None) -> InstitutionLinkResponse:
        correlation_id = ensure_correlation_id(correlation_id)
        program = self._get_tenant_program(request.tenant_id, program_id)
        now = self._now()
        ...
        program.version += 1
        self.store.update(program)
        self.audit_logger.log(event_type="program.institution_link.updated", tenant_id=program.tenant_id, correlation_id=correlation_id, actor_id=request.updated_by, details={"program_id": program.program_id, "institution_id": request.institution_id, "link_status": request.link_status.value})
        if request.link_status == LinkStatus.LINKED:
            self.event_publisher.publish(event_type="lms.program.program_institution_linked.v1", tenant_id=program.tenant_id, correlation_id=correlation_id, payload={"program_id": program.program_id, "institution_id": request.institution_id, "link_status": request.link_status.value, "linked_at": now.isoformat()})
        self.observability.increment("program_link_upsert_total")
        return self._link_response(program.institution_link)

    def replace_program_courses(self, program_id: str, request: ReplaceProgramCoursesRequest, correlation_id: str | None = None) -> ProgramCoursesMapResponse:
        correlation_id = ensure_correlation_id(correlation_id)
        program = self._get_tenant_program(request.tenant_id, program_id)
        ...
        program.updated_at = now
        self.store.update(program)
        self.audit_logger.log(event_type="program.courses.mapped", tenant_id=program.tenant_id, correlation_id=correlation_id, actor_id=request.updated_by, details={"program_id": program.program_id, "mapping_version": program.mapping_version, "course_count": len(program.course_mappings)})
        self.event_publisher.publish(event_type="lms.program.program_courses_mapped.v1", tenant_id=program.tenant_id, correlation_id=correlation_id, payload={"program_id": program.program_id, "mapping_version": program.mapping_version, "courses": [c.course_id for c in program.course_mappings], "updated_by": request.updated_by, "updated_at": now.isoformat()})
    ...
    def remove_course(self, tenant_id: str, program_id: str, course_id: str, updated_by: str, correlation_id: str | None = None) -> None:
        correlation_id = ensure_correlation_id(correlation_id)
        program = self._get_tenant_program(tenant_id, program_id)
        ...
        program.updated_at = self._now()
        self.store.update(program)
        self.audit_logger.log(event_type="program.course.unmapped", tenant_id=program.tenant_id, correlation_id=correlation_id, actor_id=updated_by, details={"program_id": program_id, "course_id": course_id})
