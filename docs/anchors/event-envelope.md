...
- `docs/data/learning-event-schema.md`
- `docs/architecture/event-driven-architecture.md`
- `docs/integrations/auth-lifecycle-events.md`
...
### 1) Event type field unified
- All three source docs used `event_type` as the event-name field — no rename required.
- `event_type` is the only allowed event-name field. No aliases.
...
|---|---|---|
| `event_type` | `event_type` | Kept as-is (already canonical across all sources) |
| `timestamp` | `timestamp` | Kept as-is (already canonical across all sources) |
...
- [x] `event_type` confirmed as single canonical event-name field (no aliases exist).
