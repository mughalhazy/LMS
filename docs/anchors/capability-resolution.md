...
> **NORMALISATION NOTE (2026-04-04):** The `country` and `segment` layers in the resolution chain below are tenant profile discriminator keys — not runtime branch conditions inside domain services. They are opaque keys used to resolve the correct config/policy layer for a given tenant context. All business rules associated with these discriminators are stored in the external config/policy store. See Master Spec §1.5 for the permitted vs prohibited use boundary. See `docs/designs/tenant-extension-model.md` for the Tenant fields that supply these discriminators.

## Purpose
This anchor defines one unambiguous capability resolution flow across:
- `docs/designs/capability-registry-service-design.md` (capability definition source)
- `docs/designs/config-service-design.md` (config override resolution)
- `docs/designs/entitlement-service-design.md` (entitlement decisioning)
...
## Architectural Contract Cross-Reference: MS-CONFIG-01 — No Runtime Branching (MS§3.2)

**Contract name:** MS-CONFIG-01 (authoritative definition in `docs/designs/config-service-design.md`)
**Source authority:** Master Spec §3.2

**Cross-reference rule for this anchor:** The `country` and `segment` discriminators in the resolution chain above are opaque lookup keys passed to the config resolver. They are not branch conditions. Services consuming the resolution output of this chain MUST NOT branch on these discriminators in their own business logic. Behavioral variation is entirely expressed through the values returned by the config layer — not through in-service conditionals.

See `docs/designs/config-service-design.md` MS-CONFIG-01 for the full rule, prohibited pattern, and enforcement requirement.
