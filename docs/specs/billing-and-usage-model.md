...
- **Regional pricing** derived from tenant billing country and contract currency. *(Implementation note: regional pricing rules are stored in the config service's country_profile layer and resolved via the config resolution chain — they are not hardcoded inside the billing service. See `docs/designs/config-service-design.md` and Master Spec §1.5.)*
