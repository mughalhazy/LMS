...
**Two-part delivery mechanism — both required:**

1. **Adapter substitution for provider differences.** Country-specific providers (payment processors, communication channels, tax authorities) are handled by adapter substitution in `/integrations/`. Adding a new country's payment provider means adding a new adapter — not modifying any service.

2. **Config layer values for behavioral differences.** Country-specific behavioral parameters (pricing, regulatory thresholds, feature limits, communication templates) are stored as config layer values in the config service's `country` layer. Adding a new country means adding its config layer — not modifying any service.

**What "no code change" means precisely:**
- Adding `country_code: "NG"` (Nigeria) to a new tenant must cause zero changes to any file outside `/integrations/` and the config store.
- If a core service file must be edited to support a new geography, that is a MS-SCALE-01 violation.

**What a violation looks like:**
- A service file containing `if country_code == "PK":` or `if country_code in ["UK", "US"]:`.
- A new country requiring a new service deployment variant or database schema change.
- A country-specific override embedded in business logic rather than expressed as a config layer value.

**Relationship to other contracts:**
- MS-CONFIG-01 (no runtime branching) is the service-level mechanism that enables MS-SCALE-01.
- MS-ADAPTER-01 (adapter isolation) is the integration-level mechanism that enables MS-SCALE-01.
- MS-MARKET-01 (market enforcements as capabilities) ensures market requirements are portable across geographies.

**Why this rule exists:** MS§13 criterion 9 is the ultimate test of platform portability. A platform that requires code changes per country is an agency project, not a global platform. MS-SCALE-01 makes this constraint explicit, named, and enforceable.
