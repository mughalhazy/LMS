...
> **NORMALISATION NOTE (2026-04-04):** The `country` and `segment` parameters in this service's API (`resolveKey` context fields) are opaque tenant profile discriminator keys passed through from the Tenant entity (`country_code`, `segment_type`). This service does not implement country-specific or segment-specific logic. It uses these as lookup keys to retrieve the appropriate pre-configured config layer from the external store. No business rules for country or segment are embedded in the resolver — they reside in the config store as retrievable layers. This is consistent with Master Spec §1.5 (permitted entitlement inputs) and §3.1 (config-driven behavior).
...
## Architectural Contract: MS-CONFIG-01 — No Runtime Branching (MS§3.2)

**Contract name:** MS-CONFIG-01
**Source authority:** Master Spec §3.2
**Scope:** This contract applies to this service AND to every service that consumes its output. It is stated here as the authoritative definition; `docs/anchors/capability_resolution.md` carries a cross-reference.

**Rule:** No service MAY contain runtime branching logic conditioned on `segment_type`, `country_code`, `plan_type`, or any other tenant profile discriminator. Behavioral variation MUST come exclusively from the output of the config resolution chain (`global → country → segment → plan → tenant`). Inline conditionals on these discriminators inside service business logic are a platform defect.

**What this prohibits:**
```python
# PROHIBITED — runtime branching inside a service
if tenant.country_code == "PK":
    send_via_jazzcash()
elif tenant.segment_type == "school":
    apply_school_workflow()
```

**What this requires:**
- Services read behavioral parameters from config resolution output and execute the resolved value.
- Country/segment/plan discriminators are passed as opaque context keys into the config resolver, which returns the appropriate pre-stored behavior parameters.
- Any behavior that varies by country, segment, or plan must be expressed as a config layer value in the config store — not as a conditional in service code.

**Determinism guarantee:** Given identical config resolution output, a service's behavior must be identical regardless of which tenant profile produced that output. The service must be unaware of which discriminator values were used to resolve the config.

**Why this rule exists:** MS§3.2 requires deterministic, hardcoded-workflow-free behavior. Inline branching creates untestable country/segment-specific code paths, makes the platform impossible to extend to new markets without code changes, and violates the Global Scalability Contract (MS-SCALE-01 in `ARCH_01_core_system_architecture.md`).
