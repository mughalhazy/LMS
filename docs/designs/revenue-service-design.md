...
### BC-REV-01 — Revenue Risk Surfacing (BOS§10.1 / GAP-015)

> **BOS source:** `workspace/foundation/behavioral-spec.md` §10.1

**Rule:** The Revenue Service MUST proactively identify and emit revenue risk signals to operators — not merely make data available for querying. Revenue risk detection must be automatic, not dashboard-dependent.

**Specification:**
- The Revenue Service already publishes `revenue.anomaly.detected` events (Section 4.6). This contract mandates the minimum set of anomalies that MUST trigger this event:

| Revenue Risk Signal | Trigger Condition | Required Event |
|---|---|---|
| Unpaid installments overdue | Installment overdue by ≥ 7 days | `revenue.anomaly.detected` with `type: unpaid_installment` |
| Subscription lapse risk | Subscription renewal due in ≤ 3 days with failed prior payment attempt | `revenue.anomaly.detected` with `type: renewal_at_risk` |
| Revenue decline | Month-to-date revenue < prior same-period by ≥ 15% | `revenue.anomaly.detected` with `type: revenue_decline` |
| Churn signal | ≥ 3 enrollment cancellations in same course in 7 days | `revenue.anomaly.detected` with `type: churn_signal` |

- Each `revenue.anomaly.detected` event payload MUST include:
  - `anomaly_type`
  - `severity` (`high` / `medium` / `low`)
  - `tenant_id`
  - `affected_entities` (list of affected student/course/subscription IDs)
  - `suggested_action` (plain-text suggested next step for the operator)
- The operations-os service subscribes to `revenue.anomaly.detected` and surfaces these as CRITICAL or IMPORTANT items in the operator's Daily Action List (BC-OPS-03).
- Detection thresholds are config-driven (config service) — the values above are defaults, overrideable per tenant.
