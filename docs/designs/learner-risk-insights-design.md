...
### BC-RISK-01 — Learner-Facing Progress Push (BOS§9.1 / GAP-011)

> **BOS source:** `workspace/foundation/behavioral-spec.md` §9.1

**Rule:** The learner risk system MUST proactively surface progress state, gaps, and improvement suggestions directly to the LEARNER — not only to teachers and admins.

**Specification:**
B6P04 currently routes all alerts to Teacher Channels and Admin Channels (Section 9). This contract extends the alert routing to include a learner-facing channel for learner-appropriate insights.

**Learner-facing insight contract:**
- When a risk finding reaches `elevated` or `critical` level, the learner must receive a proactive message — not via a teacher intermediary, but directly.
- Learner-facing insights must be reframed from risk language to empowerment language:

| Internal Risk Finding | Learner-Facing Message |
|---|---|
| `engagement: elevated` | "You've been away for a while — here's where you left off + suggested next step" |
| `performance: elevated` | "Your scores in [topic] suggest you'd benefit from reviewing [lesson] — try it now?" |
| `dropout: critical` | "Your progress has stalled — your instructor has been notified and will reach out" |

- Learner-facing insights must include:
  - A specific suggested next action (not just a warning)
  - A direct link or command to take that action
  - A motivational framing — not a deficit framing
- Learner-facing insights must be delivered via the learner's preferred channel (in-app, WhatsApp, email) per their notification preference config.
- Learner-facing delivery is opt-out (default-on) per BC-WF-01.
- The learner must never be shown the internal risk score or probability — only the human-readable narrative and suggested action.
