...
- `addon_flags`: unique, sorted add-on keys to improve deterministic downstream processing.
...
> Note: `addon_flags` can be normalized into `tenant_addons(tenant_id, addon_key)` for high-cardinality querying. Keep inline JSON/array if query volume is low to remain lightweight.
...
## See also
- `docs/anchors/tenant-contract.md` � canonical tenant contract (field names, isolation rules, addon_flags schema)
