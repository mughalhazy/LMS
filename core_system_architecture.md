# EMS Backend System Architecture (Multi-tenant, Cloud-native, API-first, Event-driven)
...
| ems_command_api_services | Domain microservices exposing API-first command/query endpoints (courses, enrollments, assessments, certifications, notifications) with strict per-tenant data ownership. | api_gateway, identity_access_service, tenant_context_service, transactional_datastores, cache_layer |
| event_bus_streaming_platform | Durable pub/sub backbone for domain events, async workflows, retries, DLQ handling, and cross-service integration. | schema_registry, tenant_context_service, ems_command_api_services, workflow_orchestrator |
| workflow_orchestrator | Runs long-lived, event-driven business processes (enrollment approval, reminder cadence, recertification lifecycle) using sagas/state machines. | event_bus_streaming_platform, ems_command_api_services, notification_service |
...
| cache_layer | Low-latency cache for tenant-scoped sessions, authorization decisions, catalog lookups, and hot query results. | ems_command_api_services, query_reporting_api |
...
| observability_platform | Centralized logs, metrics, traces, SLO monitoring, and tenant-aware operational telemetry. | service_mesh, kubernetes_platform, ems_command_api_services, event_bus_streaming_platform |
...
| disaster_recovery_resilience_service | Coordinates cross-region replication, failover orchestration, and resilience playbooks for critical EMS services. | transactional_datastores, object_storage_data_lake, kubernetes_platform, observability_platform |
