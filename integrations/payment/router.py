...
from .base_adapter import normalize_tenant


class PaymentProviderRouter:
    """Country-configured payment provider selection (no hardcoded provider)."""

    def __init__(self, country_provider_config: dict[str, str], adapters: list[PaymentAdapter]) -> None:
        self._country_provider_config = country_provider_config
        self._adapters = {adapter.provider_key: adapter for adapter in adapters}

    def resolve(self, tenant: object) -> PaymentAdapter:
