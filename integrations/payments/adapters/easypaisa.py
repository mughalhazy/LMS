...
class EasyPaisaAdapter:
    ...
    def process_payment(
        ...
        return PaymentResult(
            ok=True,
            status="success",
            payment_id=f"ep_{tenant.tenant_id}_{invoice_id or 'manual'}_{amount}",
            provider=self.provider_key,
            invoice_id=invoice_id,
        )
    ...
    def verify_payment(self, *, payment_id: str, tenant: TenantPaymentContext) -> PaymentVerificationResult:
        return PaymentVerificationResult(
            ok=payment_id.startswith("ep_"),
            status="verified" if payment_id.startswith("ep_") else "failed",
            payment_id=payment_id,
            provider=self.provider_key,
            error=None if payment_id.startswith("ep_") else "verification_failed",
        )
