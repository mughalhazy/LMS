...
// ── Derived constants (static data — computed once) ───────────────────────

const PUBLISHED_COUNT  = COURSES.filter(c => c.status === "published" && c.pub_status === "published").length
const DRAFT_COUNT      = COURSES.filter(c => c.status === "draft").length
const ARCHIVED_COUNT   = COURSES.filter(c => c.status === "archived").length
const COURSE_MAX_DATE  = COURSES.reduce((max, c) => {
  const d = new Date(c.updated); return d > max ? d : max
}, new Date(0))
...
function CompletionCell({ pct }: { pct: number | null }) {
  ...
  return (
    <div className="flex items-center gap-1.5 flex-wrap">
      ...
      {pct < 50 && (
        <span className="text-[9px] font-bold uppercase tracking-[0.05em] px-1.5 py-0.5 rounded-full bg-[var(--red-bg)] border border-[var(--red-bd)] text-[var(--red)]">
          At risk
        </span>
      )}
    </div>
...
function SortHeader({
  ...
  onSort: (col: string) => void; align?: "left" | "right" | "center"
}) {
  ...
  return (
    <TableHead
      ...
      onClick={() => onSort(col)}
    >
      <span className={cn("inline-flex items-center gap-0.5", align === "center" && "justify-center w-full")}>
        {label}
        {active ? (
          currentDir === "asc"
...
// ── Filter dropdown (desktop) — styled popup replacing native <select> ────

function FilterDropdown({
  label, value, onChange, options, noActiveStyle = false,
}: {
  label: string
  value: string
  ...
  const currentLabel = value ? (options.find(o => o.value === value)?.label ?? label) : label
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className={cn(
        "h-8 px-2 rounded-[7px] border text-xs font-medium",
        "inline-flex items-center gap-1.5 outline-none cursor-pointer transition-colors whitespace-nowrap",
        active
      ...
      <DropdownMenuContent
        align="start"
        className="min-w-[168px] rounded-[14px] shadow-[var(--sh-md)] p-1.5"
      >
        {options.map(opt => (
          <DropdownMenuItem
            key={opt.value}
...
export default function CoursesPage() {
  ...
  const filtered = useMemo(() => {
    let rows = COURSES
    if (!showArchived && statusFilter !== "archived") rows = rows.filter(c => c.status !== "archived")
    ...
    if (dateFilter) {
      const ref = new Date(COURSE_MAX_DATE)
      if (dateFilter === "30d") ref.setDate(ref.getDate() - 30)
      else if (dateFilter === "90d") ref.setDate(ref.getDate() - 90)
      else if (dateFilter === "6m")  ref.setMonth(ref.getMonth() - 6)
      rows = rows.filter(c => new Date(c.updated) >= ref)
    }
    ...
    return rows
  }, [query, statusFilter, deliveryFilter, instructorFilter, sortCol, sortDir, showArchived, dateFilter])

  // Reset mobile list when filters change
  useEffect(() => { setMobileLimit(10) }, [filtered])

  const hasFilters = query || statusFilter || deliveryFilter || instructorFilter || dateFilter
  ...
  const clearFilters = useCallback(() => {
    setQuery(""); setStatus(""); setDelivery(""); setInstructor(""); setDateFilter(""); setPage(1); setShowArchived(false)
  }, [])
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <Sheet open={filterOpen} onOpenChange={setFilterOpen}>
        <SheetContent side="bottom" className="rounded-t-[18px] px-5 pb-8 pt-5 max-h-[80vh]">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-bold uppercase tracking-[0.06em] text-[var(--ink-4)]">Last updated</label>
              <select
                className={cn(SELECT_CLS, dateFilter ? "border-[var(--lms-accent)] bg-[var(--accent-lt)] text-[var(--lms-accent)]" : "border-border")}
                value={dateFilter}
      ...
      <main className="p-4 pb-28 md:pb-7 md:p-7 md:max-w-[1100px] md:mx-auto">
        ...
        <div className="flex items-start justify-between mb-4 md:mb-5 animate-fade-up">
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--green-bg)] border border-[var(--green-bd)] text-[11px] font-semibold text-[var(--green)]">
                <span className="w-1.5 h-1.5 rounded-full bg-[var(--green-md)] shrink-0" />
                {PUBLISHED_COUNT} published
              </span>
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--accent-lt)] border border-[rgba(91,91,214,0.2)] text-[11px] font-semibold text-[var(--lms-accent)]">
                <span className="w-1.5 h-1.5 rounded-full bg-[var(--lms-accent)] shrink-0" />
                {DRAFT_COUNT} awaiting publish
              </span>
            </div>
          </div>
        ...
        <div className="hidden md:flex bg-white border border-border rounded-[14px] px-3.5 py-2.5 items-center gap-2 mb-2 shadow-[var(--sh-xs)] animate-fade-up flex-wrap">
          ...
          />
          <FilterDropdown
            label="Delivery" value={deliveryFilter}
            onChange={v => { setDelivery(v); setPage(1) }}
            options={[
          ...
          />
          <FilterDropdown
            label="Instructor" value={instructorFilter}
            onChange={v => { setInstructor(v); setPage(1) }}
            options={[
              { value: "",            label: "All courses" },
              { value: "assigned",    label: "Has instructor" },
              { value: "unassigned",  label: "No instructor" },
            ]}
          />
          <FilterDropdown
            label="Updated: any time" value={dateFilter}
          ...
          <div className="flex items-center gap-1.5 text-[11px] font-semibold text-[var(--ink-4)] whitespace-nowrap">
            ...
            <FilterDropdown
              ...
              options={[
                ...
                { value: "enrollments_desc",  label: "Enrollments ↓" },
          ...
          {!showArchived && statusFilter !== "archived" && (
            <button
              ...
              className="text-[11px] font-semibold text-[var(--ink-4)] hover:text-[var(--lms-accent)] transition-colors whitespace-nowrap"
            >
              · {ARCHIVED_COUNT} archived hidden
            </button>
          )}
          {showArchived && statusFilter !== "archived" && (
            <button
              onClick={() => setShowArchived(false)}
              className="text-[11px] font-semibold text-[var(--lms-accent)] whitespace-nowrap hover:underline"
            >
              · Hide archived
            </button>
          )}
        ...
        <div className="md:hidden mb-3 flex flex-col gap-2">
          ...
          <p className="text-[11.5px] text-[var(--ink-4)] font-medium px-0.5">
            {filtered.length} {filtered.length === 1 ? "course" : "courses"}
            {!showArchived && statusFilter !== "archived" && (
              <button onClick={() => setShowArchived(true)} className="ml-2 text-[var(--ink-4)] font-semibold hover:text-[var(--lms-accent)] transition-colors">
                · {ARCHIVED_COUNT} archived hidden
              </button>
            )}
            {showArchived && statusFilter !== "archived" && (
              <button onClick={() => setShowArchived(false)} className="ml-2 text-[var(--lms-accent)] font-semibold">
                · Hide archived
              </button>
            )}
            {hasFilters && (
              ...
              <Table>
                <TableHeader className="bg-[var(--subtle)]">
                  <TableRow className="border-b border-border hover:bg-transparent">
                    ...
                    <SortHeader col="status"      label="Status"       currentCol={sortCol} currentDir={sortDir} onSort={handleSort} align="center" />
                        ...
                        <TableCell className="text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            {getChipState(course).state === "draft" && (
                              <button className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--accent-lt)] border border-[rgba(91,91,214,0.2)] text-[11px] font-semibold text-[var(--lms-accent)] hover:bg-[rgba(91,91,214,0.12)] transition-colors whitespace-nowrap">
                                <Icon name="publish" size="xs" color="primary" />
                                Publish
                              </button>
                            )}
                          </div>
