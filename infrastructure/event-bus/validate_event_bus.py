...
def _parse_simple_yaml(path: Path) -> dict[str, Any]:
    ...
    data: dict[str, Any] = {}
    for key in ("event_name", "event", "event_topic", "topic", "producer_service", "owner"):
...
def build_catalog() -> tuple[dict[str, Any], list[str]]:
    ...
    for event_file in sorted(SERVICES_DIR.glob("*/events/*")):
        ...
        event_name = str(raw.get("event_name") or raw.get("event") or event_file.stem.replace(".event", "")).strip()
        producer = _normalize_service_name(str(raw.get("producer_service") or raw.get("owner") or service_name), service_name)
        topic = str(raw.get("topic") or raw.get("event_topic") or _normalize_topic(event_name, service_name)).strip()
